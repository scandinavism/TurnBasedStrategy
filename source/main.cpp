...
int IRRCALLCONV main(int argc, char* argv[])
{
    ...
    */
    ...
    device->setWindowCaption(L"Terra Normanorum");//меняем заголовок окна. Название временное и кодирует стадию разработки
   ...
   for (int j=0;j<J_SIZE/J_CHUNKSIZE;j++)//for each chun
       {
        ...
        //for each chunk we make ONE foliage MetaBatch from separate Tiles
         ...
         if (VisMa->TileMap[i*I_CHUNKSIZE+x][j*J_CHUNKSIZE+y].Height==2)
            {
